...
def get_model(num_users, num_items, num_genres, latent_dim, regs=[0.000], reg_layers=[0.000, 0.000, 0.000, 0.000]):
    # Input variables
    ...
    merged_vector = merge([user_latent, item_latent, genre_latent], mode = 'concat')


    layer1 = Dense(int(float(latent_dim * 3)/2), kernel_regularizer= l2(reg_layers[1]), activation='relu', name="layer1")(merged_vector)
    layer2 = Dense(int(float(latent_dim * 3)/4), kernel_regularizer= l2(reg_layers[2]), activation='relu', name="layer2")(layer1)
    layer3 = Dense(int(float(latent_dim * 3)/8), kernel_regularizer= l2(reg_layers[3]), activation='relu', name="layer3")(layer2)

    # Final prediction layer
    prediction = Dense(10, activation='softmax', init='lecun_uniform', name='prediction')(layer3)
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        if epoch %verbose == 0:
            ...
            print('Train Iteration %d [%.1f s]\txe = %.4f [%.1f s]\tmae = %.4f\tcat acc = %.4f \t5k acc = %.4f'
                  % (epoch,  t2-t1, train_loss[0], time()-t2, train_loss[1], train_loss[2], train_loss[3]))
